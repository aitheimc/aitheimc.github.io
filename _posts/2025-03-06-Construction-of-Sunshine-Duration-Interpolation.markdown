...
`일조시간`은 *태양의 직사광이 지표면에 비친 시간을 의미하며* 농업, 에너지, 관광 등 다양한 분야에서 중요한 기상 *요소이다.* ~~이지만,~~ *그러나* ~~현재 기상 예보 데이터에서~~ *기상청 데이터에서 일조시간을* 제공*하지 않아* ~~되지 않는~~ *활용에* 한계가 있다. 특히, *일조시간은* 지형적 특성과 구름량에 따라 공간적 변동성이 크지만, *기상* 관측소 *간* 간격이 넓어 정확한 분포 파악~~이~~*하기* 어렵다. *이에* 본 연구에서는 하늘 상태 정보와 지형의 음영기복도를 결합하여 보다 정밀한 `일조시간` *추정하고* 이를 토대로 *`일조기산 보간도`*를 구축하고자 한다.
<!--more-->
...
## 1. 활용 데이터
일조시간 추정을 위해 사용한 데이터는 아래 표와 같음 *[확인 요망] 데이터 종류에 대한 설명이 좀더 구체적이였으면 합니다.*
...
## 2. 구축 프로세스
*일조시간 보간도 구축 프로세스는 아래 그림과 같음* *[확인 요망] 그림을 설명하는 문구가 좀더 풍성했으면 합니다.*
<div style="text-align: center">
    <img src="../images/jhg/0304_process.png" style="max-width: 1000px; height: auto;" alt="프로세스">
</div>

### 2.1 일조시간/하늘상태 데이터 공간보간 처리
